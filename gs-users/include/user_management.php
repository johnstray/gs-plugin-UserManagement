...
<table class="highlight">
  ...
  <tbody>
    <?php foreach ($all_users as $username) {
      ...
      $USR = (isset($user['USR'])) ? strtolower($user['USR']) : '';
      $NAME = (isset($user['NAME'])) ? $user['NAME'] : '';
      $EMAIL = (isset($user['EMAIL'])) ? $user['EMAIL'] : '-- '.i18n_r('NONE').' --';
      $USERGROUP = (isset($user['USERGROUP'])) ? ucwords(str_replace('-',' ',$user['USERGROUP'])) : '-- '.i18n_r('NONE').' --';
        # TODO: Check if group exist, if not set to --none--
        # TODO: Get group details and output actual group name, not based on slug.
    ...
    <tr>
      <td style="padding:10px;">
        ...
        <a title="" href="load.php?id=<?php echo USRMGMT; ?>&action=user-editor&user=<?php echo $USR; ?>" style="text-decoration:none;display:block;line-height:15px;">
          <?php echo $NAME; ?></a>
      ...
      <td class="delete">
        <a class="delconfirm noajax" href="load.php?id=<?php echo USRMGMT; ?>&action=delete-user&user=<?php echo $USR; ?>" title="Delete User: <?php echo $USR; ?>" ><i class="fa fa-trash-o"></i></a></td>
    </tr>
    <?php } ?>
  </tbody>
